...
const bodySchema = z.object({
  ...
  content: clinicalCaseContentSchema,
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json({ error: "Invalid clinical case", details: parsed.error.flatten() }, { status: 400 });
    }
    ...
    if (current.resource_type !== "case") return NextResponse.json({ error: "Not a clinical case" }, { status: 400 });
