...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!parsed.success) return NextResponse.json({ error: "Invalid procedure", details: parsed.error.flatten() }, { status: 400 });
    ...
    if (current.resource_type !== "procedure") return NextResponse.json({ error: "Not a procedure" }, { status: 400 });
